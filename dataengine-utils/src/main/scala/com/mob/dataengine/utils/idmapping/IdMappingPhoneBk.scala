...
case class PhoneBkParams(day: String = DateUtils.currentDay(), full: Boolean = false)

case class PhoneInfo(abnormalFlag: Int, devices: Seq[DeviceInfo])

object IdMappingPhoneBk {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val defaultParams: PhoneBkParams = PhoneBkParams()
    val projectName = s"IdMappingPhoneBk[${
      DateUtils.currentDay()
    }]"

    val parser = new OptionParser[PhoneBkParams](projectName) {
      head(s"$projectName")
      opt[String]('d', "day")
 ...
 */
case class IdMappingPhoneBk(@transient spark: SparkSession, p: PhoneBkParams) extends Logging {

  import org.apache.spark.sql.functions._
  import spark.implicits._
  import IdMappingPhoneBk._
  ...
  private val headers = spark.table(HIVE_TABLE_DM_PHONE_MAPPING_V3_BK).schema.fieldNames diff
    Array("day") mkString ","
  ...
  def run(): Unit = {
    val ver = getLastPartition(HIVE_PHONE_DEVICE_TRACK_DF, day)
    val ver2 = if (p.full) "1 <> 1" else getLastPartition(HIVE_TABLE_DM_PHONE_MAPPING_V3_BK, day)
    val diDF = sql(
      s"""
         |select phone, device_list, update_time
         |from   $HIVE_PHONE_DEVICE_TRACK_DF
         |where  $ver
    ...
    // incr表的格式化
    val incrTb = "incr_tb"
    val df = diDF.select(md5($"phone").as("phone_md5"), $"phone",
      ...
      .selectExpr("phone_md5", "phone", "tm_index.timestamp as bk_tm", "tm_index.index as device_index",
        "device", "update_time")
    ...
    sql(
      s"""
         ...
         |from (
         |      select $headers
         |           , row_number() over(partition by phone_md5,phone order by update_time desc) as rn
         |      from (
         |            select $headers
         |            from   $incrTb
         |
         ...
         |
         |            select $headers
         |            from   $HIVE_TABLE_DM_PHONE_MAPPING_V3_BK
         |            where  $ver2
         |            ) as t1
         |      ) as t2
         |where rn = 1
    ...
    sql(
      s"""
         |insert overwrite table $HIVE_TABLE_DM_PHONE_MAPPING_V3_BK partition (day = '$day')
         |select $headers
         |from   $resTb
         |""".stripMargin)
  }
