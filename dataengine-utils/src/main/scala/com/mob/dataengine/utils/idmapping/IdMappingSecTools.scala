...
import com.mob.dataengine.utils.idmapping.hive.{AggregateIdTypeV2, DeviceMappingSec, IdMappingSec2, IdMappingSecExternalFull, IdMappingSec}
...
object IdMappingSecTools {
  def main(args: Array[String]): Unit = {
    val defaultParams: ParamsSec = ParamsSec()
    val projectName = s"IdMappingSecTools[${DateUtils.currentDay()}]"
    ...
    val parser = new OptionParser[ParamsSec](projectName) {
      ...
      opt[String]('i', "id_types")
        .text("device|mcid|ieid|ifid|pid|isid|snid")
        .required()
    ...
    parser.parse(args, defaultParams) match {
      case Some(params) =>
        val spark: SparkSession = SparkSession
          .builder()
          .appName(s"id-mapping-sec-tools")
        ...
        spark.sql("create temporary function Md5EncryptArrayForJhMcIdArray " +
          "as 'com.mob.udf.Md5EncryptArrayForJhMcIdArray'")
        spark.sql("create temporary function Md5EncryptArray as 'com.mob.udf.Md5EncryptArray'")
        spark.sql("create temporary function McidArrayMapStringEncrypt as 'com.mob.udf.McidArrayMapStringEncrypt'")
        ...
        spark.sql("create temporary function aes_encrypt as 'com.mob.udf.AesEncrypt'")
        spark.sql("create temporary function pid_encrypt_array as 'com.mob.udf.PidEncryptArray'")

        params.action match {
          case "hive" =>
            params.idTypes.split(",").foreach {
              case "device" =>
                new DeviceMappingSec(spark, params.day, params.test).run()
              case idType@("ifid" | "ieid" | "isid" | "snid" | "oiid") =>
                val tools = new IdMappingSec(spark, params.day, idType, params.test)
                tools.prepare()
                tools.run()
              case idType@("pid" | "mcid") =>
                val tools = new IdMappingSec2(spark, params.day, idType, params.test)
                tools.prepare()
                tools.run()
              case "external_id" =>
                val tools = new IdMappingSecExternalFull(spark, params.day, params.once, params.test)
                tools.prepare()
                tools.run()
              case idType =>
                spark.stop()
                throw new Exception(s"incorrect idType:[$idType]," +
                  s"only support[device,mcid|pid|ifid|isid|snod|oiid]")
            }
          case action =>
  ...
  // mcid_mapping/pid_mapping中的device_plat字段需要合并下
  // d1表示全量, d2表示增量
