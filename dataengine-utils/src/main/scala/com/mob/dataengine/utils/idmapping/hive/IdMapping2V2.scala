...
class IdMapping2V2(@transient spark: SparkSession, day: String, idType: String, test: Boolean = false)
  extends IdMappingBaseV2(spark, day, idType, test) {

  val splitClause: String = if (tmFields.nonEmpty) {
    (tmFields ++ md5Fields).map(f =>
      s"""
  ...
  override val transformSql: String =
    s"""
       |select ${idType}_md5, $idType, $selectFromCollapsedCols, duid, update_time, $day as day, device_plat
       |from (
       |  select ${idType}_md5, $idType, $collapseClause, duid, update_time, $day as day, device_plat
       |  from (
       |    select
       |      ${idType}_md5,
       |      $idType,
  ...
  override val insertSql: String = {
    val c1 =
      s"""
         |select ${idType}_md5, $idType, $selectFromCombinedCols,
    ...
    s"""
       ...
       |from (
       |  select ${idType}_md5, $idType, $empty2NullClause, update_time,
       |    device_plat, '$day' day
       |  from (
       |    $c1
       |  ) tmp2
