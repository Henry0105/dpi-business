...
case class CateringOffineStyle(cxt: Context) extends AbstractDataset(cxt) {
  ...
  override def _dataset(): DataFrame = {
    ...
    onlineProfile2FinanceAdapter(Map("total" -> "catering_dinein_cnt", "time" -> "catering_dinein_time_detail"),
    ...
    sql(
      s"""
         ...
         |) as tmp
      """.stripMargin)
      .toDF("device", "cateid_36", "day")
      .filter("length(cateid_36)>1")
  }

  def getTagIdMapping(): Map[String, String] = {
    Map(
      "catering_dinein_cnt_weekly" -> "4468_1000",
      "catering_dinein_cnt_monthly" -> "4469_1000",
      "catering_dinein_time_detail_weekly" -> "4478_1000",
      "catering_dinein_time_detail_monthly" -> "4479_1000"
    )
  }
}
