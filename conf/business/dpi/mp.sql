...
SELECT tag, lower(concat_ws('\\\\;m\\\\;o\\\\;b\\\\;',tag,'like',concat_ws(',',collect_set(pattern)))) as pattern, max(id) as tag_group_id
FROM (
      SELECT tag, if(url_key = '', url_regexp, concat(host,'',path,'|',url_key))as pattern, id
